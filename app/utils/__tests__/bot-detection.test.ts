...
import { isAIBot, isBot, isSearchEngineBot } from "../bot-detection";
...
describe("Bot Detection", () => {
	describe("isBot", () => {
		...
		it("should detect AI/LLM bots", () => {
			expect(isBot("Mozilla/5.0 (compatible; ChatGPT-User; +https://openai.com/chatgpt)")).toBe(
				true
			);
		...
		it("should detect social media crawlers", () => {
			expect(
				isBot("facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)")
			).toBe(true);
		...
		it("should detect SEO tools", () => {
			expect(
				isBot("Mozilla/5.0 (compatible; SemrushBot/7~bl; +https://www.semrush.com/bot/)")
			).toBe(true);
			expect(isBot("Mozilla/5.0 (compatible; AhrefsBot/7.0; +http://ahrefs.com/robot/)")).toBe(
				true
			);
		});
		...
		it("should NOT detect regular browsers", () => {
			...
			expect(
				isBot("Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:89.0) Gecko/20100101 Firefox/89.0")
			).toBe(false);
			expect(
				isBot(
	...
	describe("isSearchEngineBot", () => {
		it("should detect search engines", () => {
			...
			expect(
				isSearchEngineBot("Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)")
			).toBe(true);
			expect(
				isSearchEngineBot(
		...
		it("should NOT detect non-search engine bots", () => {
			expect(
				isSearchEngineBot("Mozilla/5.0 (compatible; ChatGPT-User; +https://openai.com/chatgpt)")
			).toBe(false);
			expect(
				isSearchEngineBot(
					"Mozilla/5.0 (compatible; SemrushBot/7~bl; +https://www.semrush.com/bot/)"
				)
			).toBe(false);
		});
	});
	...
	describe("isAIBot", () => {
		it("should detect AI/LLM bots", () => {
			expect(isAIBot("Mozilla/5.0 (compatible; ChatGPT-User; +https://openai.com/chatgpt)")).toBe(
				true
			);
			expect(isAIBot("Mozilla/5.0 (compatible; GPTBot/1.0; +https://openai.com/gptbot)")).toBe(
				true
			);
		...
		it("should NOT detect non-AI bots", () => {
			...
			expect(
				isAIBot("Mozilla/5.0 (compatible; SemrushBot/7~bl; +https://www.semrush.com/bot/)")
			).toBe(false);
		});
	});
});
